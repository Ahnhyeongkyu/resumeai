...
let cachedQrDataUri: string = '';
async function getWatermarkQrDataUri(): Promise<string> {
  if (cachedQrDataUri) return cachedQrDataUri;
  cachedQrDataUri = await QRCode.toDataURL('https://getresumeai.site?ref=pdf-qr', {
    width: 80,
    margin: 0,
...
function ResumeDocument({ resume, personalInfo, template, isFree, qrDataUri }: ResumeDocumentProps) {
      ...
      <Page size="A4" style={styles.page}>
        ...
        {isFree && (
          <View style={styles.watermarkContainer}>
            <View>
              <Text style={styles.watermarkText}>Made in 2 minutes - getresumeai.site</Text>
              <Text style={styles.watermarkSubtext}>Pay-once AI resume builder. $9, no subscription.</Text>
            </View>
            {qrDataUri && <Image src={qrDataUri} style={styles.watermarkQr} />}
          </View>
        )}
      </Page>
    </Document>
...
export async function generatePDF(
  ...
  const buffer = await renderToBuffer(
    <ResumeDocument
      ...
      template={template}
      isFree={isFree}
      qrDataUri={isFree ? await getWatermarkQrDataUri() : undefined}
    />
  );
